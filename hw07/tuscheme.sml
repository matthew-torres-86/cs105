...
(*                                                               *)
(*****************************************************************)
exception notLiteral of string
exception doLater of string

fun literal (BOOLV b)     = booltype
  | literal (SYM ds)      = symtype
  | literal (NUM b)       = inttype
  | literal (NIL)         = raise doLater "NIL"
  | literal (PAIR (a, b)) = raise doLater "PAIR"
  | literal (PRIMITIVE e) = raise notLiteral "PRIMITIVE"
  | literal (CLOSURE e)   = raise notLiteral "CLOSURE"
  | literal (ARRAY e)     = raise notLiteral "ARRAY"

(* type checking for {\tuscheme} ((prototype)) 375 *)
fun typeof (e, kinds, types) = 
    let 
      fun ty (LITERAL v) = literal v
        | ty (VAR x) = 
          (find (x, types))
        | ty (SET (x, e)) =
            if eqType((find (x, types)), ty e) then ty e
                else raise TypeError "SET IllTyped"
        | ty (IFX (e1, e2, e3)) = 
          (case ((ty e1), (ty e2), (ty e3))
              of (tau1, tau2, tau3) =>
                if eqType (tau2, tau3) andalso eqType(booltype, tau1)
                  then tau2
                else raise TypeError "IF IllTyped")
            (* | _ => raise TypeError "IllTyped" *)
        | ty (WHILEX (e1, e2)) = 
          (case ((ty e1), (ty e2))
            of (tau1, tau2) => if eqType (booltype, tau1) then unittype
              else raise TypeError "WHILE IllTyped")
          (* | _ => raise TypeError "IllTyped") *)
        | ty (BEGIN es) = 
        ...
        | ty (APPLY (f, args)) = 
          (case (ty f)
            of FUNTY(formals, result) => 
              let val argTypes = map ty args 
              in if eqTypes (argTypes, formals) then result
                else raise TypeError "APPLY unequal IllTyped"
              end
            | _ => raise TypeError "APPLY IllTyped")
        | ty (LETX (LET, binds, e)) = 
          let val (vars, vals) = ListPair.unzip binds 
            in typeof (e, kinds, bindList (vars, (map (fn (x) => ty x) vals), types))
            end 
        | ty (LETX (LETSTAR, [], body)) = typeof(body, kinds, types)
        | ty (LETX (LETSTAR, b::bs, body)) = 
          typeof (LETX(LET, [b], LETX(LETSTAR, bs, body)), kinds, types)
        | ty (LETRECX (bs, e)) = (* LETRECX  of ((name * tyex) * exp) list * exp *)
          if (List.all (fn ((n, t), expr) => 
                        (case (expr) 
                          of (LAMBDA (xs, body)) => eqType(asType(t, kinds), t)
                          | _ => false)) bs)
          then 
            let val (vars, exprs) = ListPair.unzip bs 
                val (names, tys) = ListPair.unzip vars
                val newTypes = bindList(names, tys, types)
            in typeof(e, kinds, newTypes)
            end
          else raise TypeError "IllTyped"
        | ty (LAMBDA (xs, e)) = 
          let val (vars, vals) = ListPair.unzip xs 
            in FUNTY(vals, typeof (e, kinds, bindList (vars, (map (fn (x) => asType(x, kinds)) vals), types)))
            end 
(*| TYLAMBDA of name list * exp
  | TYAPPLY  of exp * tyex list *)
        | ty (TYLAMBDA (names, e)) = raise LeftAsExercise "TYLAMBDA"
          (* let val ftv = fretyvarsGamma() *)
          

        | ty (TYAPPLY (e, tyexs)) = raise LeftAsExercise "TYLAMBDA"
          (* freetyvarsgamma and instantiate *)
    in ty e end
      
fun typdef (e, kinds, types) = 
  let
    fun defty (VAL (n, e))        = 
      if not (isbound (n, types)) then
        (bind (n, typeof (e, kinds, types), types), typeString (typeof(e, kinds, types)))
      else 
        (case (find (n, types), typeof (e, kinds, types))
          of (tau1, tau2) =>
            if eqType (tau1, tau2) then (types, typeString tau1)
            else raise TypeError "VAL IllTyped")
      | defty (VALREC (n, t, e))  = 
        let val tau = asType(t, kinds) 
        in (case (e) 
              of (LAMBDA(xs, body)) => 
                if eqType (tau, typeof(e, kinds, bind(n, tau, types))) 
                then (bind(n, tau, types), typeString tau)
                else raise TypeError "VALREC exp and t IllTyped"
              | _ => raise TypeError "VALREC IllTyped")
        end
      | defty (EXP (e))           = 
        (types, typeString (typeof (e, kinds, types)))
      | defty (DEFINE (f, rtau, (formals, body))) = 
        let val (names, tys) = ListPair.unzip formals 
        in defty(VALREC(f, FUNTY(tys, rtau), LAMBDA(formals, body)))
        end 
  in defty e
  end
(* type declarations for consistency checking *)
