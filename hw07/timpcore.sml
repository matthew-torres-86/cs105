...
(* function [[ty]], checks type of expression, given $\itenvs$ ((prototype)) 357 *)
      | ty (AAT (a, i))        = 
          (case ((ty a), (ty i))
            ...
            | _ => raise TypeError ("array-at expects array and int"))
      | ty (APUT (a, i, e))    = 
          (case ((ty a), (ty i), (ty e))
            of (ARRAYTY INTTY, INTTY, INTTY) => ARRAYTY INTTY
            | (ARRAYTY BOOLTY, INTTY, BOOLTY) => ARRAYTY BOOLTY
            | _ => raise TypeError 
                  ("array-put expects array, int, and matching element"))
      | ty (AMAKE (len, init)) = 
        (case ((ty len))
            of (INTTY) => ARRAYTY (ty init)
            | _ => raise TypeError ("make-array xpects int and array"))
      | ty (ASIZE a)           = 
          (case (ty a)
            of (ARRAYTY INTTY) => INTTY
            |  (ARRAYTY BOOLTY) => INTTY
            | _ => raise TypeError ("array-size expects an array"))
