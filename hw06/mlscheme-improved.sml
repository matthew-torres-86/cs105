...
tells whether there is a free variable of that name in the expression exp
*)
fun freeIn e n =
  let fun 
      free (LITERAL v)                  = false
    | free (VAR x)                      = x = n
    | free (SET (x, e))                 = x = n orelse free e
    | free (IFX (e1, e2, e3))           = free e1 orelse 
                                          free e2 orelse 
                                          free e3
    | free (WHILEX (guard, body))       = free guard orelse
                                          free body
    | free (BEGIN es)                   = member true (map free es)
    | free (LAMBDA (xs, e))             = not (member n xs) andalso free e
    | free (e as APPLY (f, args))       = free f orelse 
                                          member true (map free args)

    | free (LETX (LET, bs, body))       = member true (map (fn (b) => free 
                                                            (snd b)) bs) 
                                          orelse
                                          not (member n (map fst bs)) 
                                          andalso free body
    | free (LETX (LETSTAR, [], body))   = free body
    | free (LETX (LETSTAR, b::bs, body))= free (LETX (LET, [b], 
                                                      LETX (LETSTAR, bs, e)))
    | free (LETX (LETREC, bs, body))    = member true (map (fn (b) => free 
                                                            (snd b)) bs) 
                                          andalso not (member n (map fst bs))
                                          orelse
                                          free body andalso
                                          not (member n (map fst bs)) 
  in free e
...
local
   ...
   val if_xs = IFX (VAR "x", VAR "y", VAR "z")
   val while_xs = WHILEX (VAR "x", VAR "y")

   val lambda_y = LAMBDA (["y"], APPLY (VAR "equal?", [VAR "x", VAR "y"]))
   val lambda_x = LAMBDA (["x"], lambda_y)

   val let_x_t = LETX (LET, [], VAR "x")
   val let_x_f = LETX (LET, [("x", LITERAL (NUM 1))], VAR "x")
   val letrec_x_t = LETX (LETREC, [], VAR "x")
   val letrec_x_f = LETX (LETREC, [("x", LITERAL (NUM 1))], VAR "x")
   val letstar_x_t = LETX (LETSTAR, [], VAR "x")
   val letstar_x_f = LETX (LETSTAR, [("x", LITERAL (NUM 1))], VAR "x")
   (* More local definitions can go here for unit testing... *)
 in
 val () =
     Unit.checkAssert "freeIn literal false"
     (fn () => not (freeIn lit_1 "x"))
  val () =
     Unit.checkAssert "freeIn var true"
     (fn () => freeIn var_x1 "x")
 val () =
...
val () =
     Unit.checkAssert "lambda y y false"
     (fn () => not (freeIn lambda_y "y"))
val () =
     Unit.checkAssert "lambda y x true"
     (fn () => freeIn lambda_y "x")

val () =
     Unit.checkAssert "lambda x y false"
     (fn () => not (freeIn lambda_x "y"))
val () =
     Unit.checkAssert "lambda y equal? true"
     (fn () => freeIn lambda_x "equal?")

val () =
     Unit.checkAssert "if x y z true"
     (fn () => freeIn if_xs "x")
val () =
     Unit.checkAssert "while x y true"
     (fn () => freeIn while_xs "x")

val () =
     Unit.checkAssert "let x true"
     (fn () => freeIn let_x_t "x")
val () =
     Unit.checkAssert "let x false"
     (fn () => not(freeIn let_x_f "x"))
val () =
     Unit.checkAssert "letstar x true"
     (fn () => freeIn letstar_x_t "x")
val () =
     Unit.checkAssert "letstar x false"
     (fn () => not (freeIn letstar_x_f "x"))
val () =
     Unit.checkAssert "letrec x true"
     (fn () => freeIn letrec_x_t "x")
val () =
     Unit.checkAssert "letrec x false"
     (fn () => not (freeIn letrec_x_f "x"))
...
(***** Part 2 *****)
fun improve ((xs, e), env) = 
          ((xs, e), List.filter (fn (x) => freeIn e (fst x)) env)
...
fun eval (e, rho) =
  ...
  let fun ev (LITERAL v) = v
        ...
        | ev (WHILEX (guard, body)) = 
            if projectBool (ev guard) 
            then (ev body; ev (WHILEX (guard, body)))
            else BOOLV false
        | ev (BEGIN es) =
               ...
               of PRIMITIVE prim => prim (e, map ev args)
                | CLOSURE clo    =>
                    (* apply closure [[clo]] to [[args]] ((mlscheme)) 317b *)
                     ...
                     in  eval (body, bindList (formals, map ref actuals
                                              , savedrho))
...
(* if any expression in [[values]] is not a [[lambda]], reject the [[letrec]] S367a *)
                ...
                val rho' = bindList ( names
                                    , map (fn _ => ref (unspecified())) values
                                    , rho)
